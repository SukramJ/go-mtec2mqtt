...
// render edit controls for writable registers. Order matches the YAML.
func (c *Coordinator) Registers() []state.RegisterInfo {
	out := make([]state.RegisterInfo, 0, len(c.deps.Catalog.All))
...
// ErrValueParse) to the caller for HTTP status mapping.
