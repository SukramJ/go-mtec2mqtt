...
go-mtec2mqtt. It is modeled on the
...
gates: `go vet ./...`, `gofumpt` formatting, `golangci-lint run ./...`, and
`go test -race -count=1 ./...` — bundled locally as `make check` — plus the CI workflow in
`.github/workflows/ci.yml`. AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: go-mtec2mqtt is partly developed with AI assistance
