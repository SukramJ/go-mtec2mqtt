...
case "$(uname -m)" in
	...
	*) die "unsupported architecture: $(uname -m) (need x86_64, aarch64, or armv7)" ;;
esac
...
# --- version resolution -----------------------------------------------------
#
# GitHub release tags for this project carry a leading 'v' (v1.8.0, …); a
# handful of early releases (e.g. 1.0.0) predate that convention and were
# tagged bare. Release asset filenames, by contrast, never carry the 'v'
# (go-mtec2mqtt-1.8.0-linux-amd64.tar.gz). So the tag (used in the download
# URL) and the version (used in the asset filename) are two different
# strings and are tracked separately from here on: TAG and VERSION.
...
resolve_latest_version() {
	...
	# Return the tag verbatim (e.g. "v1.8.0") — callers derive the bare
	# version from it.
	printf '%s' "$tag"
}
...
if [ -z "$VERSION" ]; then
	...
	VERSION="${TAG#v}"
	ok "latest is ${BOLD}${VERSION}${RESET} (tag ${TAG})"
else
	# Manually pinned version: accept either "1.2.3" or "v1.2.3" on the
	# command line. Try the 'v'-prefixed tag first (current convention);
	# fall back to the bare tag below on a clean 404 for pre-'v' releases.
	VERSION="${VERSION#v}"
	TAG="v${VERSION}"
...
fetch_release_assets() {
	# Download both the archive and the checksum file from the release
	# at the given tag. Succeeds only when both downloads succeed; a 404
	# on either (curl -f) makes this return non-zero cleanly so the
	# caller can fall back to the other tag spelling.
	local tag="$1"
	local base="https://github.com/${REPO}/releases/download/${tag}"
	curl -sSfL --retry 3 -o "${WORK}/${ARCHIVE}" "${base}/${ARCHIVE}" \
		&& curl -sSfL --retry 3 -o "${WORK}/SHA256SUMS" "${base}/SHA256SUMS" \
		|| return 1
	RELEASE_BASE="$base"
}

info "downloading ${ARCHIVE}"
if fetch_release_assets "$TAG"; then
	ok "found release at tag ${TAG}"
elif [ "$TAG" != "$VERSION" ] && fetch_release_assets "$VERSION"; then
	ok "found release at tag ${VERSION} (pre-'v'-prefix release)"
else
	die "could not download release assets for version ${VERSION} — tried tags '${TAG}' and '${VERSION}': https://github.com/${REPO}/releases"
fi
...
if [ -f "$SYSTEMD_UNIT" ]; then
	SYSTEMD_UNIT_BACKUP="${SYSTEMD_UNIT}.bak"
	warn "${SYSTEMD_UNIT} exists — backing up to ${SYSTEMD_UNIT_BACKUP} before it is overwritten"
	cp -p "$SYSTEMD_UNIT" "$SYSTEMD_UNIT_BACKUP"
fi

info "writing systemd unit ${SYSTEMD_UNIT}"
cat > "$SYSTEMD_UNIT" <<EOF
# Generated by go-mtec2mqtt install.sh ${VERSION}
# This file is regenerated on every install/upgrade run and is
# unconditionally overwritten — the previous version was just saved to
# ${SYSTEMD_UNIT}.bak. Put local customizations in a drop-in instead:
